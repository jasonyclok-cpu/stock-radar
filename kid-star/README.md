...
- **每日任務**:主頁任務卡,今日完成 3 個回合可領 +5 ⭐(每日重置)
- **錯題特訓**:家長後台一掣生成 —— 由最近錯題抽一個回合俾小朋友重練(小朋友端只見普通闖關畫面,唔會標明「錯題」;唔影響關卡解鎖)
- **英文默書**(✏️ 小遊戲,10⭐):讀出英文字 + 顯示中文意思,砌亂序字母串返個字;砌錯唔懲罰,可以再聽(詞庫喺 `src/data/dictationWords.js`,加 `{ en, zh }` 就得)
- **故事小劇場**(🎬 小遊戲,免費):動畫繪本式閱讀理解 —— emoji 場景識郁、廣東話逐句朗讀+卡拉OK高亮,睇完即答理解題(答啱 +1⭐)。故事庫喺 `src/data/stories.js`,加故事方法檔案內有說明
